  ...
  for (ulong e = this->trainConfig.startingEpoch; e < numEpochs && !this->stopRequested.load(); e++) {
    ...
    for (ulong batchStart = 0; batchStart < numSamples; batchStart += batchSize) {
      ...
      for (ulong fetchStart = batchStart; fetchStart < batchEnd; fetchStart += fetchSize) {
        ...
        // Fetch window samples via provider
        ...
        // Distribute this fetch window across GPUs
        ulong samplesPerGPU = thisFetchSize / this->numGPUs;
        ulong remainder = thisFetchSize % this->numGPUs;
        ...
        QtConcurrent::blockingMap(
          ...
          [this, &fetchSamples, &gpuLosses, e, numEpochs, numSamples, &gpuCumulativeSamples](const GPUWorkItem& item) {
        ...
        // Update cumulative counters after each window
  ...
  for (ulong b = 0; b < numBatches; b++) {
    ...
    QtConcurrent::blockingMap(&this->workerPool, workItems, [this, &batch, &gpuResults](const GPUWorkItem& item) {
      gpuResults[item.gpuIdx] = this->gpuWorkers[item.gpuIdx]->testSubset(
        SamplesView<T>(batch.data() + item.startIdx, item.endIdx - item.startIdx));
    });
...
void CoreGPU<T>::mergeGradients()
{
  ...
  QtConcurrent::blockingMap(&this->workerPool, gpuIndices, [this, &allWeights, &allBiases](size_t gpuIdx) {
  ...
  // Write merged gradients back to all workers in parallel
  QtConcurrent::blockingMap(&this->workerPool, gpuIndices, [this, &totalWeights, &totalBiases](size_t gpuIdx) {
