...
namespace CNN {
  ...
  template <typename T>
  using SampleProvider = std::function<Samples<T>(const std::vector<ulong>& sampleIndices,
                                                   ulong batchSize, ulong batchIndex)>;
  ...
  inline SampleProvider<T> makeSampleProvider(const Samples<T>& samples) {
    return [&samples](const std::vector<ulong>& sampleIndices,
                      ulong batchSize, ulong batchIndex) -> Samples<T> {
