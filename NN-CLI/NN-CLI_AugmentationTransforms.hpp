...
namespace NN_CLI
{
  ...
  struct ElasticDeformationConfig {
      float alpha = 0.0f; // Deformation intensity (0 = disabled)
      float sigma = 5.0f; // Gaussian smoothness of deformation field
  };
  ...
  struct AugmentationTransforms {
      ...
      float randomErasing = 0.0f; // Max area fraction to erase (0 = disabled, 0.33 = up to 33%)
      float hueShift = 0.0f; // Max hue shift as fraction of 360° (0 = disabled, 0.1 = ±36°)
      float scaling = 0.0f; // Max scale deviation from 1.0 (0 = disabled, 0.2 = range 0.8×–1.2×)
      ElasticDeformationConfig elasticDeformation; // Elastic deformation (alpha = 0 = disabled)
  };
