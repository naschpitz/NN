#ifndef NN_CLI_ANNLOADER_HPP
#define NN_CLI_ANNLOADER_HPP

#include "NN-CLI_IOConfig.hpp"

#include <ANN_Core.hpp>
#include "Common/Common_Mode.hpp"
#include "Common/Common_Device.hpp"
#include <ANN_ActvFunc.hpp>
#include <ANN_LayersConfig.hpp>

#include <json.hpp>

#include <optional>
#include <string>
#include <vector>

namespace NN_CLI
{

  class ANNLoader
  {
    public:
      //-- Methods --//

      // Load ANN model from config file (convenience).
      static ANN::CoreConfig<float> loadModel(const std::string& configFilePath,
                                              std::optional<Common::ModeType> modeType = std::nullopt,
                                              std::optional<Common::DeviceType> deviceType = std::nullopt);

      // Load ANN model from pre-parsed JSON.
      static ANN::CoreConfig<float> loadModel(const nlohmann::json& json,
                                              std::optional<Common::ModeType> modeType = std::nullopt,
                                              std::optional<Common::DeviceType> deviceType = std::nullopt);

      // Load ANN model from .nnmodel package (JSON + binary parameters).
      static ANN::CoreConfig<float> loadModelPackage(const nlohmann::json& json, const std::vector<char>& binParams,
                                                     std::optional<Common::ModeType> modeType = std::nullopt,
                                                     std::optional<Common::DeviceType> deviceType = std::nullopt);

      // Load ANN samples from JSON (supports image paths when ioConfig.inputType/outputType is IMAGE)
      static ANN::Samples<float> loadSamples(const std::string& samplesFilePath, const IOConfig& ioConfig,
                                             ulong progressReports = 1000);

      // Load ANN inputs from JSON (batch: "inputs" array; supports image paths when ioConfig.inputType is IMAGE)
      static std::vector<ANN::Input<float>> loadInputs(const std::string& inputFilePath, const IOConfig& ioConfig,
                                                       ulong progressReports = 1000);
  };

} // namespace NN_CLI

#endif // NN_CLI_ANNLOADER_HPP
