...
Training produces a `.nnmodel` package (tar archive containing `model.json` + `params.bin`). This package is generated by `--mode train` and is the required format for `--mode predict` and `--mode test` (plain JSON configs are rejected for non-train modes). The package can be created from a trained model using:
