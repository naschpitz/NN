...
# Running inference
ANN-CLI --config <model_file> --mode inference --input <input_file> [options]
...
| `--mode` | `-m` | Mode: `train`, `inference`, or `test` (overrides config file) |
...
| `--input` | `-i` | Path to JSON file with input values (inference mode) |
...
- **inference**: Run inference using `--config` (trained model) with a single input.
...
### Configuration File (Inference Mode)
...
- `mode`: Operation mode (optional, default: `inference`) — *can be overridden by `--mode`*
...
This file is generated by `--mode train` and can be used directly with `--config` for `--mode inference` and `--mode test`.
...
```bash
ANN-CLI --config trained_model.json --mode inference --input test_input.json
```

### Running inference on GPU

```bash
ANN-CLI --config trained_model.json --mode inference --device gpu --input test_input.json
```
