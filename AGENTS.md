...
NN/
  ...
  NN-Server/         HTTP inference server (Civetweb) with a thread-safe CorePool over ANN/CNN cores.
...
From the repo root, configure once into a build dir, then build:
```bash
cmake -B build && cmake --build build -j$(nproc)
```
Toggle components (dependencies are validated — CNN requires ANN):
```bash
cmake -B build -DBUILD_NN_SERVER=OFF -DBUILD_NN_CLI=ON && cmake --build build -j$(nproc)
```
A single component can also be built from its own directory (each has its own
`CMakeLists.txt` and a legacy `build.sh`). Build directories match `build*/` and
are gitignored (e.g. the existing `build-verify/`).

OpenCLWrapper is a submodule — after cloning: `git submodule update --init --recursive`.
...
- **NN-Server** — Civetweb HTTP inference server; `CorePool` manages ANN/CNN core
